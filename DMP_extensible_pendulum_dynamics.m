...
subplot(3,1,1)
plot(time,[Y(1,:,i)' T(1,:,i)']); % DMP trained position
title('y')

subplot(3,1,2)
plot(time,[Y(2,:,i)' T(2,:,i)']); % DMP trained velocity
title('yd')

subplot(3,1,3)
plot(time,[Y(3,:,i)' T(3,:,i)']); % DMP trained acceleration
title('ydd')
...
for i = 2:size(Y, 2)
   ydd_i = squeeze(Y(3,i,:));
   [u,q_ddot, q_dot, q, end_effector, end_effector_accel] = robot_dynamics(ydd_i, q, q_dot, m_s, m_c, dt);
   u_hist(:,i-1) = u;
   q_hist(:,i) = q;
   q_dot_hist(:,i) = q_dot;
   q_ddot_hist(:,i) = q_ddot;
   end_effector_hist(:,i) = end_effector;
   EE_accel_hist(:,i) = end_effector_accel; 
   if mod(i,10)==0
       figure(2);
...
subplot(2,1,1)
plot(time, Y(1,:,1), time, end_effector_hist(1,:)-end_effector_hist(1,1))
title('Desired vs  Y Trajectory')
legend('Desired Y', 'End Effector Y', 'Location', 'southeast')

subplot(2,1,2)
hold on
plot(time, Y(1,:,2), time, end_effector_hist(2,:)-end_effector_hist(2,1))
...
figure(6); 
subplot(2,1,1)
plot(time, Y(3,:,1), time, EE_accel_hist(1,:))
title('desired vs actual acceleration y')
legend('desired', 'actual')
subplot(2,1,2)
plot(time, Y(3,:,2), time, EE_accel_hist(2,:))
