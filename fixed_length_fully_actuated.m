...
% n_traj = 2*tau/dt+2;
l=1;
desired_traj = [zeros(1,traj_length)]'; %zeros(1, 126); zeros(1, 126);zeros(1, 126)]'; %Downward sine wave
% figure(4)
...
plot(time, q_hist(1,:)+l.*sin(q_hist(2,:))-y0)
...
%%
function [u, q_ddot, q_dot, q] = robot_dynamics(x_ddot, q, q_dot,m_s, m_c, k, dt)
...
%         0  0 0];
    ...
    J_dot = [0 l_dot*cos(theta)-l*theta_dot*sin(theta);
             0 l_dot*sin(theta)+l*theta_dot*cos(theta)];
%              0 0 0;
%              0 0 0;
%              0 0 0];
    % mass matrix from equations of motion
    M = [m_s + m_c        m_s*l*cos(theta);
        m_s*l*cos(theta)                  m_s*l^2];
    % damping matrix from EOMs
    C = [0       -m_s*l*theta_dot*sin(theta);
         0              0];
