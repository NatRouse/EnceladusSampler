...
desired_traj = zeros(traj_length,1);
% figure(4)
...
q_dot_hist = nan(length(q), size(Y,1));
q_dot_hist(:,1) = q_dot;
figure(7)
for i = 2:size(Y, 2)
    y_iminus1 = permute(Y(1,i-1,:), [3,2,1]);
    y_i =  permute(Y(1,i,:), [3,2,1]);
    y_ddi =  permute(Y(3,i,:), [3,2,1]);
   [u,q_ddot, q_dot, q] = robot_dynamics(y_ddi, q, q_dot, m_s, m_c, k, D.dt);
   u_hist(:,i) = u;
   q_hist(:,i) = q;
   q_dot_hist(:,i) = q_dot;
   ...
   if mod(i,100)==0
       figure(7);
       plot([q(1), q(1) + q(2)*sin(q(3))], [0,-q(2)*cos(q(3))]);
       axis([-5 5 -3 3]);
...
plot(time, q_hist(1,:)+q_hist(2,:).*sin(q_hist(3,:)))
legend('Desired Y', 'DMP Y', 'End Effector Y')

% subplot(2,1,2)
% hold on
% plot(time, -l*cos(q_hist(2,:)));
% legend('Desired Z', 'DMP Z', 'End Effector Z')

hold off
figure(8);
plot(time, q_hist(1,:), time, q_hist(2,:), time, q_hist(3,:))

function [u, q_ddot, q_dot, q] = robot_dynamics(y_ddot, q, q_dot,m_s, m_c, k, dt)
...
%     g = [0 0 9.81 0 0 0]';
    ...
    %F_l_external = [0; k*l_eq+m_s*g_z*cos(theta); 0]; % forces/moments due to gravity and spring in conf. space
    z_ddot = -(- y_ddot*sin(2*theta)*l^2 + 2*g + y_ddot*sin(2*theta))/(cos(2*theta) - l^2*cos(2*theta) + l^2 + 1);
    x_ddot = [y_ddot;z_ddot];
    % mass matrix in workspace (x-y-z coordinates)
    Mx = J*M*pinv(J)-J*M*pinv(J)*J_dot*q_dot*pinv(x_ddot)+J*C*q_dot*pinv(x_ddot)+J*K*q*pinv(x_ddot); %-J*F_l_external*pinv(x_ddot);

    % total force vector in workspace coordinates
%     F_z = -m_s*g_z;
%     z_ddot = (F_z + Mx(2,1)*x_ddot)/(Mx(2,2)+0.0001);
%     x_ddot = [x_ddot; z_ddot];
%     theta_ddot = -(2*l_dot*theta_dot+y_c_ddot*cos(theta))/l + M_theta/l^2;
    F_x = Mx*x_ddot;%+m_s*l*g*sin(theta);
    
    % Actuation forces (configuration space)
    u = pinv(J)*F_x; % actuation force required
    
    % joint accelerations
    q_ddot = inv(M)*(u - C*q_dot-K*q);%+F_l_external); % should pinv(J)*F_x be included, or just u?
